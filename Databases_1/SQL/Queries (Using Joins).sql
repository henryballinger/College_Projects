...
This file is doing the queries using Joins.
There is another file for demonstrating the queries using views.
*/


-- (A - B) Customers who brought RL200UPG but not TR200UPG in for repair
select cname as CustomerName from bs_customer
    join bs_repair using (customerid)
    join bs_bikes using (modelno)
    where modelno like 'RL200UPG'
minus
select cname from bs_customer
    join bs_repair using (customerid)
    join bs_bikes using (modelno)
