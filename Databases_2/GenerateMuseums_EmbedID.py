...
# It generates a javascript file that can be run in Studio3T to add documents to a collection
...
# A single Museum ID, Museum name and Museum type
# An embedded arrays of location, containing a Street Address, City, State, Zip Code, Latitude and Longitude
...
# In order to reduce dataset size down to manageable size (~1400 rows)
...
def writemuseumfile(file):
    ...
    for r in thisfile[['museumID']].itertuples(index=False):
        ...
        info = theserows[['museumName', 'museumType', 'phoneNumber']].drop_duplicates()

        # Retrieve repeating physical location rows
        location = theserows[['streetAddress', 'city', 'state', 'zipCode', 'latitude', 'longitude']]
        ...
        rec = 'db.museum_embed_byid.insert(' + entries + ')\n'
...
filename = 'data/museum_embed_byID.js'
